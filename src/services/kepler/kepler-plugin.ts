...
const getWalletAddress = async () => {
  if (IsTauri) {
    ...
    return {
      ...
      address: state.isConnected ? state.address : 'Kepler wallet not connected'
    }
  }
}
...
export const keplerPlugin: Plugin = {
  ...
  apis: [{
    type: 'tool',
    name: 'my-address',
    description: 'My wallet address (Cosmos wallet in Tauri, Kepler wallet in Web)',
  ...
  settings: TObject({}),
  title: 'Wallet Plugin',
  description: 'Cross-platform wallet utilities (Cosmos wallet for Tauri, Kepler wallet for Web)'
}
