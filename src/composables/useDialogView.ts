...
export const useDialogChain = (dialog: Ref<DialogMapped>) => {
  ...
  const chain = computed<string[]>(() =>
    dialog.value ? getChain(null, dialog.value.msg_route)[0] : []
  )
...
export const useDialogView = (
  ...
  const dialogMessages = computed(
    () => dialogsStore.dialogMessages[dialog.value.id] || []
  )
  const { model } = useDialogModel(dialog, assistant)

  const storage = useStorage()
  const { chain, historyChain, updateMsgRoute, switchChain } =
    useDialogChain(dialog)
  ...
  const messageMap = computed<Record<string, DialogMessageMapped>>(() => {
    const map = {}
    dialogMessages.value.forEach((m) => {
      map[m.id] = m
    })
  ...
  async function addInputItems (items: ApiResultItem[]) {
    const storedItems: StoredItemMapped[] = await Promise.all(
      items.map((r) => storage.apiResultItemToStoredItem(r, dialog.value.id))
    )
