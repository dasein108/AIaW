...
export function useActiveWorkspace() {
  ...
  const assistant = computed(() => assistants.value.find(assistant => assistant.id === workspaceAssistantId.value) || defaultAssistant.value)
  const lastDialogId = computed(() => userData.value.lastDialogIds[workspaceId.value])
