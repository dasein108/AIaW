...
export const useDialogMessagesStore = defineStore("dialogMessages", () => {
  ...
  // const messageMap = computed<Record<string, Record<string, DialogMessageMapped>>>(() => {
  //   // Force reactivity by accessing the keys explicitly
  //   const keys = Object.keys(dialogMessages)

  //   return keys.reduce((acc, key) => {
  //     acc[key] = dialogMessages[key].reduce((acc, m) => {
  //       acc[m.id] = m

  //       return acc
  //     }, {} as Record<string, DialogMessageMapped>)

  //     return acc
  //   }, {} as Record<string, Record<string, DialogMessageMapped>>)
  // })

  // watch(messageMap, (newMap, oldMap) => {
  //   console.log("-----useDialogMessagesStore messageMap changed", {
  //     newCount: Object.keys(newMap).length,
  //     oldCount: Object.keys(oldMap || {}).length,
  //     newKeys: Object.keys(newMap),
  //     oldKeys: Object.keys(oldMap || {})
  //   })
  // })

  // Add a watch on dialogMessages to debug
  watch(dialogMessages, (newMessages) => {
    console.log("-----useDialogMessagesStore dialogMessages changed", {
      dialogIds: Object.keys(newMessages),
      totalMessages: Object.values(newMessages).reduce((sum, msgs) => sum + msgs.length, 0)
    })
  }, { deep: true })
  ...
  async function fetchDialogMessages(dialogId: string) {
    ...
    // Ensure we trigger reactivity by using Vue's reactive methods
    console.log("-----fetchDialogMessages: fetched", data?.length, "messages for dialog", dialogId)
    ...
    console.log("-----fetchDialogMessages: stored", dialogMessages[dialogId]?.length, "messages")
