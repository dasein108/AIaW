...
import { WalletService, WalletInfo, GranteeWalletInfo } from 'src/services/authz/wallet-service'
...
export const useAuthStore = defineStore({
  ...
  actions: {
    ...
    updateIsConnected() {
      this.isConnected = this.isGranterActuallyConnected && !!this.walletInfo && !!this.granteeSigner
    },

    async createGranteeWallet(pin: string) {
      const walletService = WalletService.getInstance()
      const newGranteeData: GranteeWalletInfo = await walletService.createGranteeWallet(pin)
      this.walletInfo = { address: newGranteeData.address, mnemonic: newGranteeData.mnemonic }
    ...
    async connectGranteeWallet(encryptedMnemonic: string, pin: string) {
      const walletService = WalletService.getInstance()
      const signer = await walletService.connectGranteeWallet(encryptedMnemonic, pin)
      ...
      if (signer && !this.walletInfo) { // Recovering walletInfo if not present
        const [account] = await signer.getAccounts()
        this.walletInfo = { address: account.address, mnemonic: encryptedMnemonic }
      }
      this.updateIsConnected()
      console.log('Grantee Wallet connected, state:', { isConnected: this.isConnected, walletInfo: this.walletInfo })
    },

    async connectWithExternalSigner(signer: OfflineSigner | OfflineDirectSigner) {
    ...
    async initializeFromStorage(granterWalletSource: CosmosWallet | KeplerWallet, pin: string) {
      if (!this.walletInfo?.mnemonic) {
        console.log('[AuthStore] No grantee mnemonic in storage for initializeFromStorage')
        // Attempt to connect granter even if grantee is not there
        try {
          const granterSigner = granterWalletSource.getOfflineSigner() as OfflineDirectSigner
          await WalletService.getInstance().validateExternalSigner(granterSigner)
          this.setGranterSigner(granterSigner)
        } catch (error) {
          console.error('Error connecting granter during initializeFromStorage (no grantee):', error)
          this.setGranterSigner(null)
        }
        return
      }
      ...
      console.log('AuthStore initialized from storage, state:', { isConnected: this.isConnected, isGranterActuallyConnected: this.isGranterActuallyConnected })
    },
    ...
    async grantAgentAuthorization(granterAddress: string, agentAddress: string, msgType?: string, expiration?: Date) {
      ...
      await walletService.grantAuthorization(this.granterSigner, granterAddress, agentAddress, msgType, expiration)
      this.isGranteeActuallyAuthorized = true
    },

    async grantMsgSendAuthorization(granterAddress: string, agentAddress: string, expiration?: Date) {
      const walletService = WalletService.getInstance()
      if (!this.granterSigner) {
        throw new Error('Granter signer not available')
      }
    ...
    async revokeAgentAuthorization(granterAddress: string, agentAddress: string, msgType?: string) {
      ...
      await walletService.revokeAuthorization(this.granterSigner, granterAddress, agentAddress, msgType)
      this.isGranteeActuallyAuthorized = false
    },
  ...
  persist: {
    ...
    paths: ['walletInfo', 'isConnected', 'isGranterActuallyConnected', 'isGranteeActuallyAuthorized']
  }
})
