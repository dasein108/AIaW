...
const mapCustomProvider = (provider: any) => {
  return {
    ...provider,
    avatar: provider.avatar as Avatar || { type: 'icon', icon: 'sym_o_dashboard_customize', hue: Math.floor(Math.random() * 360) },
...
export const useProvidersStore = defineStore('providers', () => {
  ...
  const fetchCustomProviders = async () => {
    ...
    Object.assign(providersMap, providersData.reduce((acc, provider) => {
  ...
  function createCustomProvider(provider: CustomProviderMapped, options, stack = []) {
    return (modelId: string, modelOptions) => {
      ...
      if (provider.fallback_provider) {
        return createProvider(provider.fallback_provider, options, [...stack, provider.id])?.(modelId, modelOptions)
      }
      return null
    }
  }
  ...
  async function getCustomModelList(provider: CustomProviderMapped, stack = []) {
    if (stack.includes(provider.id)) return []
    const list = provider.subproviders.map(sp => Object.keys(sp.model_map)).flat()
    provider.fallback_provider && list.push(...await getModelList(provider.fallback_provider, [...stack, provider.id]))
  ...
  const modelOptions = computed(() => removeDuplicates([
    ...baseModelOptions,
    ...Object.values(providersMap).flatMap(p => p.subproviders.flatMap(sp => Object.keys(sp.model_map)))
  ]))
  const { t } = useI18n()

  async function upsertSubproviders(provider: CustomProviderMapped, subproviders: SubproviderMapped[]) {
    if (subproviders.length === 0) {
      return []
    }
    const { data: subprovidersData, error: subprovidersError } = await supabase.from('subproviders').upsert(subproviders.map(sp => ({
      ...sp,
  ...
  async function add(props: Partial<CustomProviderMapped> = {}) {
    ...
    const { data, error } = await supabase.from('custom_providers').insert({
  ...
  async function update(id: string, changes) {
    ...
    if (Object.keys(providerItem).length > 0) {
      const { data, error } = await supabase.from('custom_providers').update(providerItem).eq('id', id).select(SELECT_QUERY).single()
      if (error) {
        console.error(error)
        return
      }
  ...
  async function deleteSubprovider(providerId: string, subproviderId: string) {
    const { error } = await supabase.from('subproviders').delete().eq('id', subproviderId).eq('custom_provider_id', providerId)
    if (error) {
      console.error(error)
      return
    }
    providersMap[providerId].subproviders = providersMap[providerId].subproviders.filter(sp => sp.id !== subproviderId)
  }
