import { useChatMessagesSubscription } from 'src/composables/chats/useChatMessagesSubscription'
...
export const useChatMessagesStore = defineStore('chat-messages', () => {
  ...
  const add = async (message: Omit<ChatMessage, 'id' | 'created_at' | 'updated_at'>) => {
    const { data, error } = await supabase
      .from('messages')
      .insert({
